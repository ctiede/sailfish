...
class Patch:
    ...
    def point_mass_source_term(self, which_mass, gravity=False, accretion=False):
        ...
        if which_mass not in (1, 2, 3):
            raise ValueError("which_mass must be either 1, 2, or 3")
        ...
        with self.execution_context:
            ...
            self.lib.cbdplanet_iso2d_point_mass_source_term[self.shape](
                self.xl,
                self.xr,
                self.yl,
                self.yr,
    ...
    def buffer_source_term(self):
        ...
        """
        ng = 2
        m1, m2, p = self.physics.point_masses(self.time)
        ...
        with self.execution_context:
            ...
            self.lib.cbdplanet_iso2d_primitive_to_conserved[self.shape](
                self.primitive1,
                conserved1,
            )

            self.lib.cbdplanet_iso2d_buffer_source_term[self.shape](
                self.xl,
                self.xr,
                self.yl,
                self.yr,
    ...
    def maximum_wavespeed(self):
        ...
        m1, m2, p = self.physics.point_masses(self.time)
        with self.execution_context:
            self.lib.cbdplanet_iso2d_wavespeed[self.shape](
                self.xl,
                self.xr,
                self.yl,
                self.yr,
    ...
    def recompute_conserved(self):
        ...
        """
        with self.execution_context:
            return self.lib.cbdplanet_iso2d_primitive_to_conserved[self.shape](
                self.primitive1,
                self.conserved0,
            )
    ...
    def advance_rk(self, rk_param, dt):
        ...
        with self.execution_context:
            self.lib.cbdplanet_iso2d_advance_rk[self.shape](
                self.xl,
                self.xr,
                self.yl,
                self.yr,
...
class Solver(SolverBase):
    """
    Adapter class to drive the cbdplanet_iso2d C extension module.
    """
    ...
    def reductions(self):
        ...
        #
        # TODO : Update to track torque from planet?
        #
        ...
        # udots3_grv = [p.point_mass_source_term(3, gravity=True) for p in self.patches]
