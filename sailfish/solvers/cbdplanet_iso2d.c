...
// ============================ GRAVITY =======================================
// ============================================================================
PRIVATE double gravitational_potential( // only uses binary
    ...
    double *delta_cons)
{
    ...
    if (mass->sink_model == 4) // Central sink instead of around each point mass
    {
...
PRIVATE double sound_speed_squared(
    ...
    double x,
    double y,
    struct PointMassList *mass_list) // Only uses central binary potential
{
    switch (eos_type)
    {
...
// ============================ PUBLIC API ====================================
// ============================================================================
PUBLIC void cbdplanet_iso2d_advance_rk(
    int ni,
    int nj,
    ...
    double sink_radius2,
    int sink_model2,
    double xp, // planet (no sink)
    double yp,
    double vxp,
    double vyp,
    double massp,
    double softening_length_p,
    ...
    double density_floor)
{
    ...
    struct PointMass pl = {xp, yp, vxp, vyp, massp, softening_length_p, 0.0, 0.0, 0};
    struct PointMassList mass_list = {{m1, m2, pl}};
...
PUBLIC void cbdplanet_iso2d_primitive_to_conserved(
    int ni,
    int nj,
...
PUBLIC void cbdplanet_iso2d_point_mass_source_term(
    int ni,
    int nj,
    double patch_xl, // mesh
    double patch_xr,
...
PUBLIC void cbdplanet_iso2d_buffer_source_term(
    int ni,
    int nj,
    double patch_xl, // mesh
    double patch_xr,
...
PUBLIC void cbdplanet_iso2d_wavespeed(
    int ni, // mesh
    int nj,
    double patch_xl,
    double patch_xr,
