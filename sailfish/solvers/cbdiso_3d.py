...
class Patch:
    ...
    def __init__(
        ...
        i0, i1 = index_range
        ni, nj, nk = i1 - i0, mesh.shape[1], mesh.shape[2]
        self.lib = lib
        self.mesh = mesh
        self.xp = xp
        self.execution_context = execution_context
        self.time = self.time0 = time
        self.shape = (i1 - i0, nj, nk)  # not including guard zones  / why not ni?
        self.physics = physics
        self.options = options
        self.xl, self.yl, self.zl = mesh.vertex_coordinates(i0, 0, 0)
        self.xr, self.yr, self.zr = mesh.vertex_coordinates(i1, nj, nk)
        ...
        with self.execution_context:
            ...
            z0 = self.zl + 0.5 * mesh.dz
            z1 = self.zr - 0.5 * mesh.dz
    ...
    def cell_center_coordinate_arrays(self):
        ...
        """
        return self.coordinate_array_x, self.coordinate_array_y, self.coordinate_array_z
    ...
    def point_mass_source_term(self, which_mass, gravity=False, accretion=False):
        ...
        with self.execution_context:
            ...
            self.lib.cbdiso_3d_point_mass_source_term[self.shape](
                self.xl,
                self.xr,
                self.yl,
                self.yr,
        ...
        return cons_rate[ng:-ng, ng:-ng, ng:-ng] # think this needed extra dimension
    ...
    def maximum_wavespeed(self):
        ...
        with self.execution_context:
            self.lib.cbdiso_3d_wavespeed[self.shape](
                self.xl,
                self.xr,
                self.yl,
                self.yr,
    ...
    def recompute_conserved(self):
        ...
        """
        with self.execution_context:
            return self.lib.cbdiso_3d_primitive_to_conserved[self.shape](
                self.primitive1,
                self.conserved0,
            )
    ...
    def advance_rk(self, rk_param, dt):
        ...
        with self.execution_context:
            self.lib.cbdiso_3d_advance_rk[self.shape](
                self.xl,
                self.xr,
                self.yl,
                self.yr,
...
class Solver(SolverBase):
    ...
    def __init__(
        ...
        if type(mesh) is not PlanarCartesian3DMesh:
            raise ValueError("solver only supports 3D cartesian mesh")
        ...
        if physics.buffer_is_enabled:
            ...
            buffer_prim = [0.0] * nq # 3->nq
            buffer_outer_radius = mesh.x1  # this assumes the mesh is a centered squared    TODO: might need to alter buffer to match a non-cubic domain
        ...
        for n, (a, b) in enumerate(subdivide(ni, num_patches)):
            prim = np.zeros([b - a + 2 * ng, nj + 2 * ng, nk + 2 * ng, nq])
            prim[ng:-ng, ng:-ng, ng:-ng] = primitive[a:b]
            patch = Patch(
                time,
                prim,
                mesh,
    ...
    def solution(self):
        return concat_on_host(
            [p.primitive for p in self.patches], (self.num_guard, self.num_guard, self.num_guard) #CHECK: correct to extend here?
        )
    ...
    def reductions(self):
        ...
        da = self.mesh.dx * self.mesh.dy * self.mesh.dz
        ng = self.num_guard
        ...
        def get_field(patch, quantity, cut, mass, gravity=False, accretion=False):
            ...
            """
            x, y, z = patch.cell_center_coordinate_arrays
            r = (x**2 + y**2) ** 0.5
            R = (x**2 + y**2 + z**2) ** 0.5
            ...
            if quantity == "torque": # TODO: Edit this calculation if binary is ever out of the plane
            ...
            if quantity == "eccentricity_vector": #2D
                sigma = apply_radial_cut(patch.primitive[ng:-ng, ng:-ng, 0])
